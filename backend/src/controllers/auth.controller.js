const { signReset, verifyToken } = require('../utils/jwt')
const {
	sendVerification,
	sendOTPVerification,
...
class AuthController {
		...
		try {
			...
			if (!user.isVerified) {
				return res.status(401).json({
					message:
						'User is not verified. Please verify your email before log in.',
				})
			}
	...
	async register(req, res) {
		try {
			...
			const verifyLink = `${process.env.DOMAIN}/api/auth/verify/${verifyToken}`
	...
	logout(req, res) {
		try {
			...
			console.log(error)
			res.status(500).json({ message: 'Error log out!', error })
		}
	}

	async verifyToken(req, res) {
		try {
			const decoded = verifyToken(req.params.verifyToken)
			if (!decoded) {
				return res
					.status(401)
			...
			console.log(error)
			res.status(500).json({ message: 'Error Verify Token', error })
		}
	}
	...
	async sendOtp(req, res) {
		try {
			...
			await sendOTPVerification(email, user.name, otp)

			return res.status(200).json({ message: 'OTP sent successfully!' })
		} catch (error) {
			console.log(error)
			res.status(500).json({ message: 'Error Send OTP!', error })
		}
	}
	...
	async verifyOtp(req, res) {
		try {
			...
			console.log(error)
			res.status(500).json({ message: 'Error Verify OTP!', error })
		}
	}
